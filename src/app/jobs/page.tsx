...
export default function FindJobsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12 relative font-sans">
      {/* Thông báo Alert */}
      {message.text && (
        <div className={`fixed top-10 right-10 z-100 px-6 py-3 rounded-xl shadow-2xl border animate-in fade-in slide-in-from-right-4 ${
      ...
      <div className="bg-linear-to-br from-violet-600 to-cyan-500 pt-16 pb-20 px-4">
        <div className="max-w-7xl mx-auto text-center text-white">
          ...
          <div className="max-w-2xl mx-auto flex gap-2 p-2 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20">
            <input
              type="text"
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-10 flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-72 bg-white p-6 rounded-4xl shadow-xl border border-gray-100 h-fit sticky top-24">
           ...
           <div className="space-y-8">
             <div>
               <label className="block text-[10px] font-bold text-gray-400 uppercase mb-4">Ngân sách: ${filters.maxPrice}</label>
               <input 
                type="range" min="50" max="10000" step="50" value={filters.maxPrice}
             ...
             <div>
               <label className="block text-[10px] font-bold text-gray-400 uppercase mb-4">Sắp xếp</label>
               <select 
...
function JobCard({ job, currentUserId, setMessage }: any) {
  ...
  // (Dựa trên cấu trúc application trả về từ getAllJobsAction)
  const hasApplied = job.application?.some((app: any) => app.freelancerId === currentUserId);

  const handleApply = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.proposal) return alert("Vui lòng nhập thư giới thiệu");
    ...
    try {
      ...
      if (result.success) {
        ...
        // Có thể reload hoặc cập nhật state tại đây
    ...
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 hover:border-violet-200 transition-all group">
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          <span className="text-[10px] font-bold text-gray-400 uppercase">Ngân sách dự kiến</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-gray-50">
        ...
        <div className="flex gap-3">
          <Link href={`/jobs/${job.id}`} className="px-6 py-3 rounded-xl border border-gray-200 text-sm font-bold hover:bg-gray-50 transition-all">Chi tiết</Link>
          
          {hasApplied ? (
            <div className="flex items-center gap-2 px-6 py-3 bg-green-50 text-green-600 rounded-xl font-bold text-sm border border-green-100">
               <CheckCircle size={16} /> Đã ứng tuyển
            </div>
          ) : (
            <button 
              onClick={() => setShowApplyModal(true)}
              className="px-8 py-3 bg-violet-600 text-white rounded-xl font-bold text-sm hover:bg-violet-700 shadow-lg shadow-violet-200 transition-all uppercase"
            >
              Ứng tuyển ngay
            </button>
          )}
        ...
        <div className="fixed inset-0 z-110 flex items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center mb-8">
               <h2 className="text-2xl font-bold text-gray-900 uppercase italic">Gửi báo giá</h2>
               <button type="button" onClick={() => setShowApplyModal(false)} className="p-2 hover:bg-gray-100 rounded-full"><X size={20}/></button>
            </div>

            <div className="space-y-6">
                <div>
                   <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2">Giá chào thầu ($)</label>
